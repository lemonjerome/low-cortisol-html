...
3. consumes compiler/runtime diagnostics,
...
### 2) Compiler diagnostics feedback injection
Added compiler/runtime feedback extraction and reinjection in `orchestrator/loop_controller.py`:
...
- Exports selected backend to `COMPILOT_DEVICE` environment variable.
...
2. Compiler failure output appears in next-iteration context (via feedback injection).
